...
#ifndef MCSLBUTTON_H_
#define MCSLBUTTON_H_

#include "mcslglobal.h"
#include "mcslobj.h"

//#define MCSL_BT_CONTENT_LEN		16	/*按钮上所能显示的最多字符数*/
...
struct _mcslbutton{
	...
	//private data
	mcsl_u32 pressed_color;	/*由button控件所独有。当按键时背景色*/
	Pixmap pressed_map;	/*由button控件所独有。当按键时背景图案*/

	PUBLIC
	//所有控件共有接口
	/*该控件上是否鼠标正被按下*/
	int (*ispressed)(mcslwidget *thismcsl , mcsl_u32 type_button);
	...
	//控件独有的接口
	/*设置按钮按下颜色*/
	int (*set_pressed_color)(struct _mcslbutton *this , char *pressed_RGB);
	/*设置按钮按下背景图*/
	int (*set_pressed_map)(struct _mcslbutton *this , char **pressed_bg_xpm);

};

typedef struct _mcslbutton mcslbutton;
 ...
 * @param1:按钮上的文字(可以为空)
 */
extern mcslbutton *new_mcslbutton(mcslwidget *parent , char *str_content);

#define MCSLBUTTON(e) ((mcslbutton *)e)
